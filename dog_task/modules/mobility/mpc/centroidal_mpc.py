...
class CentroidalMPC:
    ...
    def solve_QP(self, go2:PinGo2Model, traj: ComTraj, verbose: bool = False):
        ...
        warm_start = hasattr(self, 'x_prev') and self.x_prev is not None
        if warm_start:
            qp_args['x0'] = self.x_prev
            qp_args['lam_x0'] = self.lam_x_prev
            qp_args['lam_a0'] = self.lam_a_prev

        # 4) Solve the QP with fallback on failure
        try:
            ...
            if warm_start:
                try:
                    qp_args.pop('x0', None)
                    qp_args.pop('lam_x0', None)
                    qp_args.pop('lam_a0', None)
                    sol = self.solver(**qp_args)
                except RuntimeError:
                    sol = self._fallback_solution()
            else:
                sol = self._fallback_solution()
        ...
        self.x_prev = sol["x"]
        self.lam_x_prev = sol.get("lam_x")
        self.lam_a_prev = sol.get("lam_a")
    ...
    def _fallback_solution(self):
        ...
        _log.warning("QP solver failed, using fallback stance forces")

        if hasattr(self, 'x_prev') and self.x_prev is not None:
            return {
                "x": self.x_prev,
                "lam_x": ca.DM.zeros(self.nvars),
                "lam_a": ca.DM.zeros(self.nvars),
            }

        w = 6.921 * 9.81 / 4  # weight per leg
        u_fb = np.tile([0.0, 0.0, w], 4)
        u_all = np.tile(u_fb, self.N)
        x_all = np.zeros(self.N * NX)
        x = ca.DM(np.concatenate([x_all, u_all]))
        return {
            "x": x,
            "lam_x": ca.DM.zeros(x.size1()),
            "lam_a": ca.DM.zeros(x.size1()),
        }